...
class ItemBlock:
    ...
    def __repr__(self):
        return [x.strip() for x in self.itemLines]
...
class Customer:
    ...
    def __str__( self ):
        return f"address: {self.addressBlock.__repr__()}, snail: {self.getsPrintedNotices()}, wellformed: {self.isWellFormed()}, itemcount: {len( self.items )}"
